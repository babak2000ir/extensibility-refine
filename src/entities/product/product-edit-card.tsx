import { useForm, useSelect, BaseKey } from "@refinedev/core";
...
export const ProductEditCard = ({ id }: { id: BaseKey }) => {
    ...
    const record = queryResult ? queryResult.data?.data : {};
    ...
    return (
        <form onSubmit={onSubmit}>
            ...
            <select id="category" name="category" defaultValue={record?.category.id}>
                {options?.map((option) => (
                    <option
                        key={option.value}
                        value={option.value}
